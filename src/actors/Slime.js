...
(function(){
    ...
    var walk = cc.spawn(
            cc.sequence(
                    cc.delayTime(dur/8),
                    cc.JumpBy3D.create(dur*7/8, cc.math.vec3(0, 0, 0), 30, 1)
                ),
            cc.sequence(
    ...
    var attack1 = cc.spawn(
        cc.moveBy(dur/2, cc.math.vec3(0, 0, 20)).easing(cc.easeBounceOut()),
        cc.rotateBy(dur/2, cc.math.vec3(70, 0, 0)).easing(cc.easeBounceOut())
        // cc.moveBy(dur/2, cc.p(-40, 0)).easing(cc.easeBounceOut())
        );
    attack1.retain();

    var attack2 = cc.spawn(
        cc.moveBy(dur, cc.math.vec3(0, 0, -20)).easing(cc.easeBounceOut()),
        cc.rotateBy(dur*3/4, cc.math.vec3(-70, 0, 0))
    ...
    var die = cc.spawn(
        cc.sequence(
                cc.JumpBy3D.create(dur/2, cc.math.vec3(0, 0, 0), 30, 1),
                cc.scaleBy(dur, 2, 2, 0.1)
            ),
        cc.rotateBy(dur, cc.math.vec3(-90, 0, 0))
        );
    die.retain();

    var knock = cc.sequence(
        cc.rotateBy(dur/3, cc.math.vec3(-60, 0, 0)).easing(cc.easeBackInOut()),
        cc.rotateBy(dur/2, cc.math.vec3(60, 0, 0))
        );
    knock.retain();
